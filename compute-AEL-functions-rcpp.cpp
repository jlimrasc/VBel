...
VectorXd get_dF_Rcpp(vector<VectorXd> h_list, VectorXd wi_arr, int n, int d) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    dF = dF + vi * h_list[i]; // Calculate sum for this iteration of dF
  }
  return dF;
}
...
// [[Rcpp::export]]
VectorXd compute_lambda_Rcpp(vector<VectorXd> h_list, MatrixXd H_Zth, VectorXd lam0, float a, int T, int n, int d) {
// # -----------------------------
...
// [[Rcpp::export]]
double compute_AEL_Rcpp(VectorXd th, function<MatrixXd(MatrixXd, VectorXd)> h, 
                        VectorXd lam0, float a, MatrixXd z, int T = 100) {
// # -----------------------------
// # Default values
// # -----------------------------
//       if (missing(T)) {
...
// # -----------------------------
  ...
  MatrixXd H_Zth(n,n), h_znth(1, 2);
  VectorXd h_zith(d);

  for (int i = 0; i < n - 1; i++) {
    h_zith = h(z.row(i).transpose(), th);
...
// # AEL Calculation
// # -----------------------------
  float accum = 0, log_AEL;
  
  for (int i = 0; i < n; i++) {
    accum += log(1.f + (lambda.transpose() * h_list[i])[0]);
  }
