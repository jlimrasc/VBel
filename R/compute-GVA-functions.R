...
#' @param delthh        User defined function, outputs kxp Jacobian matrix of h(zi,th) with respect to theta
#' @param delth_logpi   User-defined function, outputs px1 matrix, derivative of log prior function
...
#'
#' @examples
compute_GVA_R <- function(mu, C, h, delthh, delth_logpi, z, rho, elip, T, T2, theta, lam0, a) {
    # Initialise values
    ...
    mu_t        <- mu
    mu_arr      <- array(dim = c(dim(mu_t), T+1))
    mu_arr[,,1] <- mu_t
